...
def filter_job(latest_job: str, data: Dict[str, List[Dict[str, Union[str, float, List[str]]]]]) -> Tuple[List[Dict[str, Union[str, float, List[str]]]], List[str], Dict[str, int]]:
    ...
    for match in filtered_data:
        ...
        match_overlap_data[match['job_name']] = round(len(
            match['matching_skills']) / (len(match['matching_skills']) + len(match['missing_skills'])), 2)

    match_overlap_data = pd.DataFrame.from_dict(
        match_overlap_data, orient="index").reset_index()
...
if latest_job != "":  # only run the next bits once the user has entered a latest job
    ...
    with st.expander(label=ordered_matches[0]):
        ...
        with matches:
            st.markdown(
                "We think you **already have** the following skills and experiences that would be needed as a {}:".format(ordered_matches[0]))
    ...
    with st.expander(label=ordered_matches[1]):
        ...
        with matches:
            st.markdown(
                "We think you **already have** the following skills and experiences that would be needed as a {}:".format(ordered_matches[1]))
            for skill in match_data["matching_skills"]:
                st.markdown(":green[- " + skill + "]")
        with bar:
            st.markdown(
    ...
    with st.expander(label=ordered_matches[2]):
        ...
        with matches:
            st.markdown(
                "We think you **already have** the following skills and experiences that would be needed as a {}:".format(ordered_matches[2]))
            for skill in match_data["matching_skills"]:
                st.markdown(":green[- " + skill + "]")
        with bar:
            st.markdown(
