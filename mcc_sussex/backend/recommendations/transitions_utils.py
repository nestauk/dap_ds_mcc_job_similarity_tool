...
import mcc_sussex.backend.recommendations.compare_nodes_utils as compare_nodes_utils
import mcc_sussex.backend.getters.load_data_utils as load_data
from mcc_sussex.backend.getters.embeddings import load_embeddings
from mcc_sussex.backend.recommendations import pickle_large_files
...
data_dir = f"{PROJECT_DIR}/mcc_sussex/data/"
...
with open(f"{PROJECT_DIR}/mcc_sussex/backend/config/default_transition_params.yaml", 'r') as f:
...
def transition_data_processing(
    ...
    W_skills = 0.5*sim.W_essential[j_id, viable_ids] + \
        0.5*sim.W_all_to_essential[j_id, viable_ids]
    W_work = 0.5*sim.W_activities[j_id, viable_ids] + \
        0.5*sim.W_work_context[j_id, viable_ids]
    ...
    columns['W_optional_skills'] += list(
        sim.W_all_to_essential[j_id, viable_ids])
...
def transition_data_filtering(trans_df, MIN_VIABLE, HIGHLY_VIABLE):
    ...
    trans_df.loc[trans_df.similarity <=
                 HIGHLY_VIABLE, 'sim_category'] = 'min_viable'
    trans_df.loc[trans_df.similarity > HIGHLY_VIABLE,
                 'sim_category'] = 'highly_viable'
    trans_df.loc[trans_df.similarity <=
                 MIN_VIABLE, 'sim_category'] = 'not_viable'
    trans_df['is_viable'] = trans_df['is_jobzone_ok'] & (
        trans_df['sim_category'] != 'not_viable')
...
def create_filtering_matrices(
    ...
    for i in range(N):
        ...
        for j in range(N2):
            ...
            is_jobzone_ok = np.abs(
                row_i.job_zone - row_j.job_zone) <= MAX_JOB_ZONE_DIF
            is_earnings_ok = (row_j.annual_earnings /
                              row_i.annual_earnings) > MIN_EARNINGS_RATIO
            is_safer = (row_i.risk > row_j.risk) & (
                row_i.prevalence < row_j.prevalence)
    ...
    F_min_viable = F_jobzone & (W_combined_select > MIN_VIABLE) & (
        W_combined_select <= HIGHLY_VIABLE)
    ...
    # 2nd (stricter) definition
    F_strictly_safe_desirable = F_desirable & F_strictly_safe
...
def show_skills_overlap(
    ...
    assert ((data.node_to_essential_items.loc[[
            job_j]].items_list.values[0]) != 0)
    ...
    if verbose:
        ...
        print(
            f'NLP-adjusted overlap = {w:.2f} (total combined similarity: {sim.W_combined[job_i, job_j]:.2f})')
...
class CompareFeatures():
    ...
    def __init__(self, data_folder=data_dir):
        ...
        self.work_context_vectors = np.load(
            data_folder + 'interim/work_context_features/ESCO_work_context_vectors.npy')
        self.work_context_features = pd.read_csv(
            data_folder + 'processed/work_context_vector_features.csv')
        self.work_context_features['category'] = self.work_context_features.element_id.apply(
            lambda x: int(x[4]))
        ...
        def categorise(x):
            ...
            if x == 3:
                return 'structural'
        self.work_context_features['category'] = self.work_context_features['category'].apply(
        ...
        self.esco_features_1 = pickle.load(open(
            data_folder + 'interim/work_activity_features/esco_hierarchy_codes_level_1.pickle', 'rb'))
        self.esco_features_1 = data.concepts[data.concepts.code.isin(
            self.esco_features_1)][['code', 'title']].sort_values('code').copy()

        self.esco_vectors_2 = np.load(
            data_folder + 'interim/work_activity_features/esco_hierarchy_vectors_level_2.npy')
        self.esco_features_2 = pickle.load(open(
            data_folder + 'interim/work_activity_features/esco_hierarchy_codes_level_2.pickle', 'rb'))
        self.esco_features_2 = data.concepts[data.concepts.code.isin(
            self.esco_features_2)][['code', 'title']].sort_values('code').copy()

        self.esco_vectors_3 = np.load(
            data_folder + 'interim/work_activity_features/esco_hierarchy_vectors_level_3.npy')
        self.esco_features_3 = pickle.load(open(
    ...
    def most_impactful_features(self, origin_id, destination_id, esco_level=2):
        ...
        original_sim = cosine(
            normalize(original_destination_vector.reshape(1, -1)), origin_vector)
        impacts = []
...
class SkillsGaps():
    ...
    def get_skills_scores(self, verbose=True):
        ...
        for j, row in self.trans_to_analyse.iterrows():
            ...
            self.destination_skills_id_ALL.append(
                df.destination_skill_id.to_list())
            ...
            origin_skills = df[df.origin_skill_id.apply(
                lambda x: type(x) != str)]

            # Extract the origin skill IDs, matching scores and similarity values
            self.origin_skills_id_ALL.append(
                origin_skills.origin_skill_id.to_list())
            self.origin_skills_id_score_ALL.append(
                origin_skills.score.to_list())
            self.origin_skills_id_sim_ALL.append(
                origin_skills.similarity.to_list())

        t_elapsed = time() - t
        if verbose:
            print(
                f'Time elapsed: {t_elapsed :.2f} sec ({t_elapsed/len(self.trans_to_analyse): .3f} per transition)')
    ...
    def setup(self, transition_indices=None, skills_type='destination', skill_items=None):
        ...
        if skill_items is None:
            ...
            df = self.skill_similarities_all.merge(
                data.skills[['id', 'skill_category']], left_on='skills_id', right_on='id', how='left')
            self.skill_similarities_all = self.skill_similarities_all[df.skill_category.isin(
                skill_items)]
    ...
    def prevalent_cluster_gaps(self, level='level_3', top_x=10, percentile=False):
        ...
        prevalent_clusters = self.get_most_prevalent_gaps(
            self.cluster_gaps, top_x=top_x, percentile=percentile)
    ...
    def merge_lists(self):
        ...
        skill_similarities_all.loc[skill_similarities_all.score.isnull(
        ), 'score'] = 0
    ...
    def count_and_agg_scores(self, skill_similarities_all, groupby_column):
        ...
        # Mean similarity
        skill_similarities = skill_similarities_all.groupby(
            groupby_column).mean()
        # Create the dataframe
        skill_similarities['counts'] = skill_counts['score']
        skill_similarities['stdev'] = skill_similarities_all.groupby(
            groupby_column).std()['score']
    ...
    def get_skills_gaps(self):
        ...
        skill_similarities = self.count_and_agg_scores(
            self.skill_similarities_all, 'skills_id')
    ...
    def get_cluster_gaps(self, level='level_1'):
        ...
        skill_similarities = skill_similarities.merge(
            data.concepts[['code', 'title']], left_on=level, right_on='code')
        # Clean up the dataframe
        skill_similarities = self.clean_up_df(skill_similarities)
        skill_similarities = skill_similarities[[
            'code', 'title', 'counts', 'prevalence', 'score', 'stdev']]
    ...
    def get_most_prevalent_gaps(self, skills_gaps, top_x=10, percentile=False):
        ...
        if percentile:
            df = skills_gaps[skills_gaps.prevalence >
                             np.percentile(skills_gaps.prevalence, top_x)]
...
class Upskilling():
    ...
    def __init__(self,
        ...
        self.list_of_new_skills = [skill if type(skill) == list else [
            skill] for skill in new_skillsets]
        ...
        self.origin_ids_to_row_indices = dict(
            zip(self.origin_ids, list(range(len(self.origin_ids)))))
        self.destination_ids_to_col_indices = dict(
            zip(self.destination_ids, list(range(len(self.destination_ids)))))
        self.row_indices_to_origin_ids = dict(
            zip(list(range(len(self.origin_ids))), self.origin_ids))
        self.col_indices_to_destination_ids = dict(
            zip(list(range(len(self.destination_ids))), self.destination_ids))
        ...
        activity_vector_dir = f'{data_dir}interim/work_activity_features/'
        self.element_codes_2 = np.array(pickle.load(
            open(f'{activity_vector_dir}esco_hierarchy_codes_level_2.pickle', 'rb')))
        self.normalisation_params = pickle.load(
            open(f'{activity_vector_dir}esco_hierarchy_norm_params.pickle', 'rb'))
        self.occupation_vectors_level_2_abs = np.load(
            f'{activity_vector_dir}esco_hierarchy_vectors_level_2_abs.npy')
        self.occupation_vectors_level_2 = np.load(
            f'{activity_vector_dir}esco_hierarchy_vectors_level_2.npy')
        # Variables including work context similarities into the combined measure
        esco_to_work_context_vector = pd.read_csv(
            data_dir
            + 'interim/work_context_features/occupations_work_context_vector.csv')
        esco_with_work_context = esco_to_work_context_vector[esco_to_work_context_vector.has_vector == True].id.to_list(
        )
        occ_no_work_context = set(data.occupations.id.to_list()).difference(
            set(esco_with_work_context))
        self.origin_indices_no_work_context = self.indices_of_specified_elements(
            self.origin_ids, occ_no_work_context)
        self.destination_indices_no_work_context = self.indices_of_specified_elements(
            self.destination_ids, occ_no_work_context)
    ...
    def effectiveness(self,
        ...
        for n, new_skill in enumerate(self.list_of_new_skills):
            ...
            if select_destination_ids is not None:
                selected_transition_df = transition_df[transition_df.destination_id.isin(
            ...
            if safe_definition == 'default':
                ...
                selected_transition_df = selected_transition_df[
                    selected_transition_df.is_strictly_safe_desirable]
            ...
            if significance_test_tolerance is not False:
                analysis_dict['p_value'] = wilcoxon(
                    df.counts.to_list(), correction=True).pvalue
    ...
    def add_skills_categories(df):
        ...
        df = df.merge(data.concepts[['code', 'title']], left_on='level_1', right_on='code', how='left').drop(
            'code', axis=1).rename(columns={'title': 'ESCO skill category'})
        df = data.add_field_to_skill(df, 'new_skill', 'level_2')
        df = df.merge(data.concepts[['code', 'title']], left_on='level_2', right_on='code', how='left').drop(
            'code', axis=1).rename(columns={'title': 'ESCO skill subcategory'})
        return df

    def clean_up_df(self, df):
        df.new_skill, contains_combinations = self.clean_up_list(
            df.new_skill.to_list())
        df.new_skill_label, _ = self.clean_up_list(
            df.new_skill_label.to_list())
        if not contains_combinations:
            df = self.add_skills_categories(df)
        return df
    ...
    def new_transitions(self,
        ...
        for n, new_skill in enumerate(self.list_of_new_skills):
            ...
            viable_transitions = np.where(
                (W_new_combined > MIN_VIABLE) & (W_combined_baseline <= MIN_VIABLE))
            ...
            transition_pairs_indices = [
                (viable_transitions[0][x], viable_transitions[1][x]) for x in range(len(viable_transitions[0]))]
            transition_pairs_ids = [(self.row_indices_to_origin_ids[i],
                                     self.col_indices_to_destination_ids[j]) for i, j in transition_pairs_indices]
            transition_df = get_transition_data(
                transition_pairs_ids, verbose=self.verbose)
            # Organise the dataframe
            transition_df = transition_df.drop(
                ['W_skills', 'W_work', 'W_essential_skills', 'W_optional_skills', 'W_activities', 'W_work_context'], axis=1)
            ...
            transition_df['is_viable'] = (
                transition_df['new_similarity'] > MIN_VIABLE) & transition_df['is_jobzone_ok']
            ...
            transition_df['is_new'] = transition_df['is_viable'] & (
                transition_df['baseline_viable'] == False)

            # Count new safe and desirable transitions for each occupation
            counts_safe_desirable = self.count_transitions(
                transition_df[transition_df.is_new & transition_df.is_safe_desirable])
            counts_strictly_safe_desirable = self.count_transitions(
                transition_df[transition_df.is_new & transition_df.is_strictly_safe_desirable])
            ...
            for job_i in self.origin_ids:
                ...
                if len(df) != 0:
                    for j, row in df.iterrows():
                        job_i_trans['destination_label'].append(
                            row.destination_label)
                        job_i_trans['destination_id'].append(
                            row.destination_id)
    ...
    def recalculate_similarities(self, load_data=False):
        ...
        self.new_W_essential_skills = self.recalculate_skills_similarities(
            skills_match='essential')
        self.new_W_optional_skills = self.recalculate_skills_similarities(
    ...
    def combine_similarity_measures(self, W_essential, W_optional, W_activities, W_context, params):
        ...
        W_combined = (params['p_essential_skills'] * W_essential) + (params['p_optional_skills'] * W_optional) + (
            params['p_work_activities'] * W_activities) + (params['p_work_context'] * W_context)
        # Adjust for cases where work context doesn't exist for either origin or destination occupation
        p_essential_skills_x = params['p_essential_skills'] / \
            (1-params['p_work_context'])
        p_optional_skills_x = params['p_optional_skills'] / \
            (1-params['p_work_context'])
        p_work_activities_x = params['p_work_activities'] / \
            (1-params['p_work_context'])
        for i in self.origin_indices_no_work_context:
            for j in range(len(W_combined)):
                W_combined[i][j] = (p_essential_skills_x * W_essential[i][j]) + (
                    p_optional_skills_x * W_optional[i][j]) + (p_work_activities_x * W_activities[i][j])
    ...
    def recalculate_skills_similarities(self, skills_match='optional'):
        ...
        """
        if self.verbose:
            print(f'Recalculating {skills_match} skills similarities...')
        ...
        destination_node_to_items = self.adjust_node_ids(
            destination_node_to_items, id_offset=self.n_origin_occupations)
        ...
        for new_skills in self.list_of_new_skills:
            if self.verbose:
                print(f'Adding skill(s) {new_skills} to origin occupations.')
            ...
            perturbed_origin_node_to_items.items_list = perturbed_origin_node_to_items.items_list.apply(
                lambda x: literal_eval(x))
            # Combine both origin and destination lists of skills
            node_to_items = pd.concat(
                [perturbed_origin_node_to_items, destination_node_to_items]).reset_index(drop=True)

            # suppress the warning, due to the one occupation without essential skills
            with np.errstate(divide='ignore'):
                ...
                t = time()
                if self.verbose:
                    print('Running comparisons...', end=' ')
                Comp.run_comparisons(dump=False)
                Comp.collect_comparisons()
                t_elapsed = time()-t
                if self.verbose:
                    print(f'Done in {t_elapsed:.0f} seconds!')
            ...
            origin_edges = np.array(Comp.real_edge_list)[np.where(
                np.array(i_edges) < self.n_origin_occupations)[0]]

            W_perturbed = np.zeros(
                (self.n_origin_occupations, self.n_destination_occupations))
            for edge in origin_edges:
                W_perturbed[edge[0], edge[1] -
                            self.n_origin_occupations] = W[edge[0], edge[1]]
    ...
    def adjust_node_ids(node_to_items, id_offset=0):
        ...
        node_to_items['id'] = np.array(
            list(range(0, len(node_to_items)))) + id_offset
    ...
    def recalculate_work_activity_similarities(self):
        ...
        t = time()
        if self.verbose:
            print('Recalculating work activity feature vector alignments...', end=' ')
        ...
        for new_skills in self.list_of_new_skills:
            ...
            for i, job_i in enumerate(self.origin_ids):
                ...
                origin_skillset = data.node_to_essential_items.loc[[
                    job_i]].items_list.values[0]
                ...
                for new_skill_id in new_skills:
                    ...
                    # Add the skill to the job_i feature vector
                        ...
                        element_id = np.where(
                            self.element_codes_2 == skill_code)[0][0]
                ...
                new_d = cdist(
                    new_feature_vector_norm, self.occupation_vectors_level_2[self.destination_ids, :], 'euclidean')
                new_d = (new_d - self.normalisation_params['d_MIN_LEVEL2'])/(
                    self.normalisation_params['d_MAX_LEVEL2']-self.normalisation_params['d_MIN_LEVEL2'])
    ...
    def count_transitions(self, transition_df):
        ...
        df = transition_df.groupby('origin_id').agg(
            {'destination_id': 'count'}).reset_index().rename(columns={'destination_id': 'counts'})
    ...
    def dump_results(self, filename='upskilling_results.pickle', dir=f'{data_dir}interim/upskilling_analysis/'):
        """
        Dumps the recalculated, perturbed skills matrices for later reuse
        """
        if self.verbose:
            print(f'Dumping in {dir+filename}')
        result_dict = {
    ...
    def load_results(self):
        ...
        """
        if self.verbose:
            print(f'Loading data form {self.load_data_path}')
