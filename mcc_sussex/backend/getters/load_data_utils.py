...
class Data:
    ...
    def add_field_to_occupation(self, df, occupation_id_col='id',  field_name=None, new_prefix=None):
        ...
        df_new = df.copy().merge(
            self.occ[['id', field_name]], left_on=occupation_id_col, right_on='id', how='left')
        if new_prefix is not None:
            df_new = df_new.rename(
                columns={field_name: new_prefix + '_' + field_name})
    ...
    def add_field_to_skill(self, df, skill_id_col='id',  field_name=None, new_prefix=None):
        ...
        df_new = df.copy().merge(
            self.skills[['id', field_name]], left_on=skill_id_col, right_on='id', how='left')
        if new_prefix is not None:
            df_new = df_new.rename(
                columns={field_name: new_prefix + '_' + field_name})
        if skill_id_col != 'id':
    ...
    def occupation_hierarchy(self):
        ...
        if self._occupation_hierarchy is None:
            self._occupation_hierarchy = self.read_csv(
    ...
    def isco_titles(self):
        ...
        if self._isco_titles is None:
            self._isco_titles = self.read_csv(
                self.dir + 'ISCO_occupation_titles.csv')
    ...
    def occ_risk(self):
        ...
        if self._occ_risk is None:
            self._occ_risk = self.read_csv(
                self.dir + 'ESCO_automation_risk_full.csv')
    ...
    def occ_risk_report(self):
        ...
        if self._occ_risk_report is None:
            self._occ_risk_report = self.read_csv(
    ...
    def occ_jobzones(self):
        ...
        if self._occ_jobzones is None:
            self._occ_jobzones = self.read_csv(
    ...
    def occ_earnings_and_hours(self):
        ...
        if self._occ_earnings_and_hours is None:
            self._occ_earnings_and_hours = self.read_csv(
    ...
    def occ_employment(self):
        ...
        if self._occ_employment is None:
            self._occ_employment = self.read_csv(
    ...
    def occ_remote(self):
        ...
        if self._occ_remote is None:
            self._occ_remote = self.read_csv(
    ...
    def occ_exposure(self):
        ...
        if self._occ_exposure is None:
            self._occ_exposure = self.read_csv(
                self.dir + 'linked_data/ESCO_occupations_COVID_Exposure.csv')
    ...
    def occ_clusters(self):
        ...
        if self._occ_clusters is None:
            self._occ_clusters = self.read_csv(
    ...
    def clusters_level_1(self):
        ...
        if self._clusters_level_1 is None:
            self._clusters_level_1 = self.read_csv(
    ...
    def clusters_level_2(self):
        ...
        if self._clusters_level_2 is None:
            self._clusters_level_2 = self.read_csv(
    ...
    def generate_occupation_profiles(self, export=True, save_path=None):
        ...
        occ = occ.merge(
            self.occ_exposure[['id', 'physical_proximity', 'exposure_score']], **merge_params)
    ...
    def occ_report(self):
        ...
        if self._occ_report is None:
            ...
            self._occ_report = self.occ[self.occ.id.isin(
...
class Similarities:
    ...
    def W_all_to_essential(self):
        ...
        if self._W_all_to_essential is None:
            self._W_all_to_essential = np.load(
    ...
    def W_work_context(self):
        ...
        if self._W_work_context is None:
            self._W_work_context = np.load(
                self.dir + 'sim_matrices/OccupationSimilarity_ONET_Work_Context.npy')
